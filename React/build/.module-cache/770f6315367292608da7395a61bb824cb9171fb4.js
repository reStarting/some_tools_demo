...
var ImageCarousel = React.createClass({displayName: "ImageCarousel",
	...
	handleClick: function(i){
		...
		var newItems = items.splice(i, 1);
		console.log(newItems, i);
		this.setState({items:items});
	},
	render: function() {
		var items = this.state.items.map(function(item, i){
			return React.createElement("img", {onClick: this.handleClick.bind(this, i), className: "pic", width: "400", src: item, key: item});
		}.bind(this));
		return (
			React.createElement("div", null, 
				React.createElement(ReactCSSTransitionGroup, {transitionName: "carousel", className: "slider"}, 
					items
				)	
			)
		);
